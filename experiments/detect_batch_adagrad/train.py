...
if __name__ == '__main__':
	...
	if len(sys.argv) > 1: # very very basic
		model = DetectorContainer('detect_batch_adagrad',construct(int(sys.argv[1])),int(sys.argv[1]),"adagrad")
		model.train(nb_epoch=int(sys.argv[2]), batch_size=int(sys.argv[3]), samples_per_epoch=int(sys.argv[4]))
	else:
		model = DetectorContainer('detect_batch_adagrad',construct(256),256,"adagrad")
